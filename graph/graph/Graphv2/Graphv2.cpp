...
int edgeSearch(Graph* g, int fid, int sid);
...
int timing(Graph*g) {
	...
	while (n++ < 10) {
		...
		printf("%d strongly connected components, decomposing time = %d\n\n", k, last - first);
		delGraph(&p);
		delGraph(&t);
	}
	return 0;
...
int edgeSearch(Graph* g, int fid, int sid) {
	...
	while (tmp) {
		if (tmp->vertex->point->id == sid) return 0;
		tmp = tmp->next;
	}
...
int main() {
	Graph g = { NULL, 0, 0 };
	int rc;
	while (rc = dialog(loadmsgs, NLoadMsgs)) if (!lfptr[rc](&g)) break;//if (lfptr[rc](&g)) break;
